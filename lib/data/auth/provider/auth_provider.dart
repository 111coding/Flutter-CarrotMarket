...
class AuthProvider {
  ...
  Future<bool?> login() async {
    final response = await _ref.read(httpProvider).post(
          "/login",
          body: AuthLoginDto(username: 'tester', password: '1111').toJson(),
        );
  ...
  Future<bool?> login2() async {
    final response = await _ref.read(httpProvider).post(
          "/login",
          body: AuthLoginDto(username: 'tester2', password: '1111').toJson(),
        );
    if (response.statusCode == 200) {
      jwt = response.headers["authorization"]!;
      // 로그인 하고 공통으로 관리할 정보 가져오기!
  ...
  Future<bool?> join(
      {required String nickname, required String addressFullName}) async {
  ...
  Future<bool> nicknameCheck({required String username}) async {
    final response =
        await _ref.read(httpProvider).get("/api/user/nicknameCk", query: {
      "nickname": username,
    });
    if (response.statusCode == 200) {
      final code = jsonDecode(response.body)["code"];
